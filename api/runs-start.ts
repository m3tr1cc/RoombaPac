...
import { CURRENT_MAZE_VERSION, LEGACY_MAZE_VERSION } from '../src/game/maze.js'
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    const mazeVersion = request.body.mazeVersion === CURRENT_MAZE_VERSION ? CURRENT_MAZE_VERSION : LEGACY_MAZE_VERSION
    ...
    const { error } = await database.from('roombapac_run_sessions').insert({ id: runId, player_hash: playerHash, ip_hash: ipHash, seed, maze_version: mazeVersion, expires_at: expiresAt.toISOString() })
    if (error) throw error
    response.setHeader('cache-control', 'no-store')
    return response.status(201).json({ runId, seed, issuedAt: issuedAt.toISOString(), mazeVersion })
  } catch (error) {
